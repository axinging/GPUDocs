...
```
根据PR里面的解释，这个选项是必须的，否则会导致hang。我这边nodejs测试出来，等于0会hang。
但是要注意的是，这个选项对TFJS WASM实际使用的线程数目其实并没有决定作用。真正决定TFJS WASM里面使用的线程的数目的函数，是pthreadpool_create。
所以，在PR https://github.com/tensorflow/tfjs/pull/4942/ 里面，通过下面这行代码获取的线程数目，就是TFJS WASM实际使用的线程数目。
```
const size_t get_thread_pool_size() { return backend::thread_pool_size; }
```

TFJS在通过pthreadpool_create创建线程池之前（backend.cc），使用了emscripten_num_logical_cores来获得线程池的数目。
