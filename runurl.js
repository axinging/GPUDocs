...
async function startContext(exitCondition, tracingFile, logFile) {
    let browserArgs = `--disable-dawn-features=disallow_unsafe_apis  --enable-dawn-features=record_detailed_timing_in_trace_events,disable_timestamp_query_conversion --enable-unsafe-webgpu --enable-tracing=disabled-by-default-gpu.dawn  --trace-startup-file=${tracingFile} --trace-startup-format=json`;
...
async function runURL(url, tracingFile, logFile) {
  if (url == '') {
    throw 'URL is empty';
  }
